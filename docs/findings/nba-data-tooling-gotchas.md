...
- **G League shot data**: `ShotChartDetail(league_id="20")` returns ~94k
  shots/season in the identical NBA schema — `shot-quality-study/python/
  05_gleague.py` runs the NBA-fit model on it unchanged. pbpstats'
  `get-games/gleague` API returned empty for standard season params in our
  probe; the nba_api route is the verified one.
